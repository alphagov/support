...
describe LabelledSnippet do
  ...
  context "with multiple fields" do
    it "apples only if one of the fields is set" do
      expect(snippet(on: data(f1: "abc"), fields: [:f1, :f2], label: "Fields").applies?).to be_truthy
      expect(snippet(on: data(f1: "abc", f2: "xyz"), fields: [:f1, :f2], label: "Fields").applies?).to be_truthy

      expect(snippet(on: data(), fields: [:f1, :f2], label: "Fields").applies?).to be_falsey
      expect(snippet(on: data(f1: nil), fields: [:f1, :f2], label: "Fields").applies?).to be_falsey
      expect(snippet(on: data(f1: ""), fields: [:f1, :f2], label: "Fields").applies?).to be_falsey
    end
